...
contract HalSaram is Owned{
    ...
    // 생성자
    function HalSaram(uint256 _supply, string _name, string _symbol, uint8 _decimals) {
...
contract Crowdsale is Owned {
    ...
    HalSaram public tokenReward;                        // 지불에 사용할 토큰
    ...
    function Crowdsale (
        ...
        HalSaram _addressOfTokenUsedAsReward
    ) {
        fundingGoal = _fundingGoalInEthers * 1 ether;
        price = 1 ether / _amountOfTokenPerEther;
        transferableToken = _transferableToken;
        tokenReward = HalSaram(_addressOfTokenUsedAsReward);
    }
...
contract Escrow is Owned {
    ...
    // (4) 생성자
    function Escrow (HalSaram _token, uint256 _salesVolume, uint256 _priceInEther) {
        token = HalSaram(_token);
